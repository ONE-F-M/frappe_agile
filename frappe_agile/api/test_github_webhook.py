...
Run with:
    bench run-tests --app frappe_agile \
...
class TestGithubWebhookSignature(unittest.TestCase):
    ...
    MODULE = "frappe_agile.api.github_webhook"

    def _get_module(self):
        from frappe_agile.api import github_webhook
        return github_webhook

    @patch("frappe.conf", new_callable=lambda: type("Conf", (), {"get": lambda self, k, d=None: "test-secret"})())
    def test_valid_signature_passes(self, _mock_conf):
...
class TestHandlePullRequest(unittest.TestCase):
    ...
    def test_pr_opened_assigns_reviewer(self, mock_apply):
        ...
        mock_apply.assert_called_once_with("WI-000042", "Assign Reviewer", pr_url=unittest.mock.ANY)
    ...
    def test_pr_merged_merges_pr(self, mock_apply):
        ...
        mod._handle_pull_request(payload)
        mock_apply.assert_called_once_with("WI-000042", "Merge PR", pr_url=unittest.mock.ANY)
...
class TestHandlePullRequestReview(unittest.TestCase):
    ...
    def test_changes_requested_fires(self, mock_apply):
        ...
        mock_apply.assert_called_once_with("WI-000007", "Request Changes", pr_url=unittest.mock.ANY)
...
class TestHandlePush(unittest.TestCase):
    ...
    def test_push_from_pending_execution_fires(self, _mock_get, mock_apply):
        mod = self._mod()
    ...
    def test_push_from_later_state_is_guarded(self, _mock_get, mock_apply):
    ...
    def test_push_from_wrong_state_is_guarded(self, _mock_get, mock_apply):
    ...
    @patch("frappe_agile.api.github_webhook._apply")
    @patch("frappe.db.get_value", return_value="Pending Execution")
    def test_push_extracts_from_commit_message(self, _mock_get, mock_apply):
