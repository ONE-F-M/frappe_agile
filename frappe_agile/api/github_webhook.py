...
def _handle_push(payload: dict):
    ...
    # For push events the Work Item reference is taken from the commit messages
    # or the branch name only (no PR body available).
    commits = payload.get("commits", [])
    commit_messages = " ".join(c.get("message", "") for c in commits)

    wi_name = _extract_wi_name(commit_messages, branch_name)
    if not wi_name:
        # Not every push has a WI reference — no log spam, just return.
        return
...
def _verify_signature():
    ...
    if not hmac.compare_digest(received_sig, expected_sig):
        frappe.log_error(
            ...
            message=(
                f"Expected sha256={expected_sig}, "
                f"received sha256={received_sig}. "
